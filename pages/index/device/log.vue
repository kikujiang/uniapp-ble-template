<template>
	<view class="content">
		...
		<view class="title_view">
			...
			<image src="../../../static/index/clear.png" class="search_img" @click="clear"></image>
		</view>
		...
		<view v-if="user_record_data !=null && user_record_data.length > 0" class="view_data">
			<view v-for="(item,index) in user_record_data" :key="index" class="record_content">
				...
				<view v-if="item.type == 1" class="record_content1">
					...
					<text class="record_text1">{{flag == 1?"更换气罐":"灭杀成功"}}</text>
				</view>
				
				<view v-else class="record_content1">
					<text class="record_text">{{getTime(item.time_record)}}</text>
					<!-- <text class="record_text1">{{flag == 1?"捕获成功":"灭杀成功"}}</text> -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			clear(){
				let me = this
				http.send('record/delete', 'POST', {
					device_id:this.currentId
				}, (res) => {
					if (res.code == 0) {
						console.log("清空日志返回数据为:",res.data)
