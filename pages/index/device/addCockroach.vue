...
<script>
	...
	export default {
		...
		onLoad(option) {
			let me = this
			this.initModuleMeta()
			console.log('======添加设备加载中,获取的数据为：',option)
		...
		methods: {
			...
			fetchDeviceList(){
				...
				return new Promise((resolve) => {
					http.send('user/get_device_company_list?room_id=0', 'GET', {}, (res) => {
						if (res.code == 0) {
							console.log("获取当前服务器设备列表数据为:",res.data)
							me.device_connect_list = res.data || []
						}
						resolve()
					}, (err) => {
						console.log("获取当前服务器设备列表失败，错误原因是:",err)
			...
			addDevice(){
				...
				console.log("当前设备蓝牙地址为：",this.device_mac,",设备名称为：",this.device_name,",设备位置为：",this.device_location)
				...
				http.send('Device/add_device_company', 'POST', {
					...
					if (res.code == 0) {
						console.log("添加设备返回数据为:",res.data)
			...
			initBLE(){
				console.log("============检查蓝牙状态==========")
				if(!this.hasModuleCapability('scan')) return
			...
			stopDiscovery(){
				...
				session.stopScan().then(() => {
					console.log("停止查找蓝牙设备回调成功")
					afterStop()
				}).catch((error) => {
			...
			handleScanFoundPayload(payload) {
				...
				console.log("成功添加一条数据：",result.candidate)
			},
