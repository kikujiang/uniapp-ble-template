...
export const parseResponse = (rawHex = '') => {
  ...
  if (functionCode === '04' && commandCode === '02') {
    const countDownSeconds = frameHex.length >= 10 ? parseInt(frameHex.substring(8, 10), 16) : null
    return {
      ...base,
      type: 'discharge.start.ack',
      payload: {
        success: resultCodeHex === '01',
        countDownSeconds
      }
  ...
  if (functionCode === '07' && commandCode === '01') {
    return {
      ...base,
      type: 'power.low_alert',
      payload: {
        success: true
      }
    }
